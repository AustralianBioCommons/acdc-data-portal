    ...
    const columns = config.studyColumns.map((column) => ({
            ...
            render: (_, record) => {
                if (column.contentType === 'request_access_button') {
                    const authzValue = record[config.minimalFieldMapping.authzField];
                    ...
                    return (
                        <Button
                            onClick={(ev) => {
                                ...
                                // Show confirmation modal BEFORE creating the REMS application.
                                // This prevents orphaned REMS applications when users click
                                // "Request Access" but then cancel — previously the POST to
                                // requestor (which creates the REMS application) fired before
                                // the modal appeared, so every click created an application
                                // regardless of whether the user confirmed.
                                Modal.confirm({
                                    title: 'Request access to this dataset?',
                                    content: (
                                    ...
                                    onOk: async () => {
                                        try {
                                            ...
                                            if (status === 200 || status === 201) {
                                                ...
                                                message.error(`Failed to submit access request (status: ${status}). Please try again.`);
                                            }
                                        } catch (err) {
                                            message.error('An error occurred while submitting your access request. Please try again.');
                                        }
                                    },
                                });
                            }}
